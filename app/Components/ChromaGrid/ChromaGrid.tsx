...
/*
  Based on https://reactbits.dev/components/chroma-grid
  بهینه‌سازی برای Safari/Chrome بدون تغییر در منطق/ظاهر
*/
...
export interface ChromaGridProps {
  items?: ChromaItem[];
  className?: string;
  radius?: number; // ماسک نور
  damping?: number; // دَمپ تعقیب نشانگر
  fadeOut?: number; // برگشت لایه‌ی fade
  ease?: string;
}
...
const ChromaGrid: React.FC<ChromaGridProps> = ({
  ...
  // متغیرهای CSS را سریع ست می‌کنیم (بدون ساخت tween جدید)
  const setX = useRef<SetterFn | null>(null);
  const setY = useRef<SetterFn | null>(null);

  // موقعیت جاری ماسک (روی یک آبجکت جدا که quickTo بتواند tween کند)
  const pos = useRef({ x: 0, y: 0 });
  ...
  // انیمیشن ورود کارت‌ها (سبک + will-change)
  // انیمیشن ورود کارت‌ها (سبک + will-change)
  useEffect(() => {
    if (!mounted || !rootRef.current) return;
    ...
    gsap.set(cards, { willChange: "transform,opacity", force3D: true });
    ...
    tl.from(cards, {
      y: 160,
      opacity: 0,
      stagger: 0.18, // همون ترتیب، کمی سبک‌تر
      force3D: true,
    }).add(() => {
      gsap.set(cards, { clearProps: "will-change" });
    });

    // ✅ حتماً یک تابعِ cleanup برگردون
    return () => {
      tl.kill(); // ← این باشه، نه `return tl` و نه `return tl.kill`
    };
  }, [mounted, data.length]);

  // Spotlight: quickSetter + quickTo + ticker (بدون ساخت tween روی هر move)
  useEffect(() => {
    if (!mounted || !rootRef.current) return;
    ...
    // tween-er مخصوصِ مختصات (هیچ tween جدیدی ساخته نمی‌شود)
  ...
  return (
    ...
    >
      {data.map((c, i) => {
        ...
        const ratio = centerIndex === 0 ? 0 : offset / centerIndex;
        const angle = (maxArcAngle / 3) * ratio;

        const x = radiusArc * Math.sin(angle);
        const y = radiusArc * (0.9 - Math.cos(angle));
            ...
            style={
              {
                ...
                transform: `translate(-50%, -50%) translate(${x}px, ${
                  ...
                  rotation * 0.8
                }rad) scale(${scale}) translateZ(0)`,
                zIndex: Math.round(depth * 10) + 1,
                // لایه‌ی GPU برای هر کارت
                willChange: "transform,opacity",
          ...
          >
            ...
            <div className="relative z-10 flex-1 p-[10px] box-border">
              <img
                ...
                decoding="async"
                draggable={false}
                className="w-full h-full object-cover rounded-[10px] will-change-transform"
                style={{
                  transform: "translateZ(0)",
                  WebkitTransform: "translateZ(0)",
                }}
      ...
      {/* ماسک خاکستری/روشن — نسخه‌ی سبک‌تر در Safari */}
      <div
        className="absolute inset-0 pointer-events-none z-30"
        style={{
          borderRadius: 32,
          // در Safari: فیلتر گرون رو حذف کن ولی ماسک/گرادیان بمونه
          backdropFilter: isSafariBrowser
            ? "none"
            : "grayscale(1) brightness(0.78)",
          WebkitBackdropFilter: isSafariBrowser
            ? "none"
            : "grayscale(1) brightness(0.78)",
          background: "rgba(0,0,0,0.001)",
          maskImage:
      ...
      <div
        ...
        className="absolute inset-0 pointer-events-none transition-opacity duration-[250ms] z-40"
        style={{
          // در Safari هم این یکی سبک‌تره؛ فیلتر حذف می‌شه
          backdropFilter: isSafariBrowser
            ? "none"
            : "grayscale(1) brightness(0.78)",
          WebkitBackdropFilter: isSafariBrowser
